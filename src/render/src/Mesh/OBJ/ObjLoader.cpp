...
namespace sge
{
	...
	void ObjLoader::LoadMem(EditMesh& mesh, Span<const u8> src)
	{
	...
	void ObjLoader::_loadmem(EditMesh& mesh, Span<const u8> src)
	{
		mesh.clear();
		_outMesh = &mesh;
	...
	void ObjLoader::_parseLine_f()
	{
		...
		for (size_t i = 0; i < face_vi.size(); i++) {
			if (i >= 3) {
				...
				_outMesh->index.emplace_back(static_cast<u32>(indicesStart + i - 1));
			}
			...
			_outMesh->index.emplace_back(vi);
		}
